class Route
  ...
  def validate!
    ...
    raise "Start station can`t be nil, and must have min 3 symbols" if @start_station.name.length.zero? && @start_station.name.length < 3
    raise "Finish station can`t be nil, and must have min 3 symbols" if @finish_station.name.length.zero? && @finish_station.name.length < 3
    true
  end
end